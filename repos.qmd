...
<script>
  ...
  const mount = document.getElementById("weather-table");

  //  get the latest commit touching the folder
  const commits = await fetch(
    "https://api.github.com/repos/lmacakova/weather_data_pipeline/commits?path=data/weather&per_page=1"
  ).then(r => r.json());

  const sha = commits?.[0]?.sha;
  if (!sha) return meta.textContent = "No data found.";

  //  get files changed in that commit
  const commit = await fetch(
    `https://api.github.com/repos/lmacakova/weather_data_pipeline/commits/${sha}`
  ).then(r => r.json());

  const file = commit.files
    .map(f => f.filename)
    .find(f => f.startsWith("data/weather/") && f.endsWith(".json"));

  if (!file) return meta.textContent = "No JSON in latest commit.";

  meta.textContent = `Showing ${file.split("/").pop()}`;

  const data = await fetch(
    `https://raw.githubusercontent.com/lmacakova/weather_data_pipeline/main/${file}`
  ).then(r => r.json());

  renderTableFromJson(data, mount);
}
